...
export async function POST(request: Request) {
  ...
  try {
    ...
    feedOwnerId =
      typeof body?.feedOwnerId === 'string' ? body.feedOwnerId.trim() : '';
    ...
    if (
      !feedOwnerId ||
      !userId ||
      (rawAction !== 'add' && rawAction !== 'remove')
    ) {
      return NextResponse.json(
        {
          error:
            'Missing or invalid feedOwnerId, userId, or action (must be "add" or "remove")',
        },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          err instanceof Error
            ? err.message
            : 'Failed to update premium membership',
      },
      { status: 500 },
    );
  }
