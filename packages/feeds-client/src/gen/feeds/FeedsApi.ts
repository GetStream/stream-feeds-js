...
export class FeedsApi {
    ...
    {id: string,
        ...
        const response = await this.apiClient.sendRequest<StreamResponse<GetActivityResponse>>('GET', '/api/v2/feeds/activities/{id}', pathParams ,  undefined  );
    ...
    {object_id: string,object_type: string,depth?: number,sort?: string,replies_limit?: number,limit?: number,prev?: string,next?: string,
    }
    ): Promise<StreamResponse<GetCommentsResponse>> {
        
        const queryParams = {
          object_id: request?.object_id, object_type: request?.object_type, depth: request?.depth, sort: request?.sort, replies_limit: request?.replies_limit, limit: request?.limit, prev: request?.prev, next: request?.next,
        };
    ...
    {id: string,depth?: number,sort?: string,replies_limit?: number,limit?: number,prev?: string,next?: string,
    }
    ): Promise<StreamResponse<GetCommentRepliesResponse>> {
        
        const queryParams = {
          depth: request?.depth, sort: request?.sort, replies_limit: request?.replies_limit, limit: request?.limit, prev: request?.prev, next: request?.next,
        };
        const pathParams = {
          id: request?.id,
        };
    ...
    {feed_group_id: string,feed_id: string,
        ...
        const body = {
          description: request?.description,enrich_own_fields: request?.enrich_own_fields,name: request?.name,filter_tags: request?.filter_tags,custom: request?.custom,
        }
    ...
    {source: string,target: string,delete_notification_activity?: boolean,enrich_own_fields?: boolean,
    }
    ): Promise<StreamResponse<UnfollowResponse>> {
        
        const queryParams = {
          delete_notification_activity: request?.delete_notification_activity, enrich_own_fields: request?.enrich_own_fields,
        };
        const pathParams = {
          source: request?.source, target: request?.target,
        };
